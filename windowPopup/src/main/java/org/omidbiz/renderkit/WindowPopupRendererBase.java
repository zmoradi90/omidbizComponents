    ...
    protected void doEncodeBegin(ResponseWriter writer, FacesContext context, UIComponent component) throws IOException
    {
        ...
        if (t != null && id != null)
        {
            ...
            if ("dialog".equals(type) || "button".equals(type))
            {
                ...
                overlayClose = overlayClose == null ? Boolean.TRUE : overlayClose;
                Boolean closeButton = (Boolean) component.getAttributes().get("closeButton");
                closeButton = closeButton == null ? Boolean.TRUE : closeButton;
                Boolean escKey = (Boolean) component.getAttributes().get("escKey");
                escKey = escKey == null ? Boolean.TRUE : escKey;
                // javascript
                StringBuilder sb = new StringBuilder().append("jQuery(document).ready(function(){");
                sb.append("jQuery(\"a[rel=rel_" + id.replace(":", "\\\\:")
                        + "]\").colorbox({fastIframe:false, width:\"80%\", height:\"80%\", iframe:" + iframe + ",escKey:" + escKey
                        + ", closeButton:" + closeButton + ", open:" + autoOpen + ", overlayClose:" + overlayClose + "});");
