...
import org.omidbiz.event.CurrentDateChangeEvent;
import org.omidbiz.event.CurrentDateChangeListener;
import org.omidbiz.model.CalendarDataModel;
import org.omidbiz.model.CalendarDataModelItem;
import org.omidbiz.renderkit.CalendarRendererBase;
import org.omidbiz.renderkit.JalaliCalendar;

/**
 * JSF component class
 * 
 */
public abstract class UIPersianCalendar extends UIInput implements AjaxComponent {
	...
	private final static Log log = LogFactory.getLog(UIPersianCalendar.class);
	...
	public JalaliCalendar getCalendar() {
		return new JalaliCalendar(getTimeZone(), getAsLocale(getLocale()));
//		return Calendar.getInstance(getTimeZone(), getAsLocale(getLocale()));
	...
	protected Object getDefaultValueOfDefaultTime() {
	    JalaliCalendar calendar = getCalendar();
	    calendar.set(JalaliCalendar.HOUR_OF_DAY, 12);
	    calendar.set(JalaliCalendar.MINUTE, 0);
	...
	public Date getFormattedDefaultTime() {
	    ...
	    if (defaultTime instanceof JalaliCalendar) {
		result = ((JalaliCalendar) defaultTime).getTime();
	...
	public String getPreparedDefaultTime() {
	    ...
	    result.append(calendar.get(JalaliCalendar.HOUR_OF_DAY));
	    result.append(",");
	    result.append(MINUTES_VALUE).append(":");
	    result.append(calendar.get(JalaliCalendar.MINUTE)).append("}");
		...
		try {
			...
			if (ve != null) {
					...
					ve.setValue(context.getELContext(), convert.getAsString(context, this,
							currentDate));
					return;
				} else if (ve.getType(elContext).equals(JalaliCalendar.class)) {
		...
		if (date == null) {
			return null;
				...
				return (Date) date;
					...
					if (date instanceof JalaliCalendar) {
						return ((JalaliCalendar) date).getTime();
					} else {
	...
	protected Date getDefaultPreloadBegin(Date date) {
		JalaliCalendar calendar = new JalaliCalendar(getTimeZone(), getAsLocale(getLocale()));//JalaliCalendar.getInstance(getTimeZone(), getAsLocale(getLocale()));
		calendar.setTime(date);
		calendar.set(JalaliCalendar.DATE, calendar.getActualMinimum(JalaliCalendar.DATE));
		/*
	...
	protected Date getDefaultPreloadEnd(Date date) {
		JalaliCalendar calendar = new JalaliCalendar(getTimeZone(), getAsLocale(getLocale()));
		calendar.setTime(date);
		calendar.set(JalaliCalendar.DATE, calendar.getActualMaximum(JalaliCalendar.DATE));
		/*
		 * //force recalculation calendar.getTimeInMillis();
	...
	public Date convertCurrentDate(String currentDateString) {
		...
		calendar.clear();
		calendar.set(JalaliCalendar.DATE, 1);
		int idx = currentDateString.indexOf('/');
		if (idx != -1) {
			calendar.set(JalaliCalendar.MONTH, Integer.parseInt(currentDateString
					.substring(0, idx)) - 1);
			calendar.set(JalaliCalendar.YEAR, Integer.parseInt(currentDateString
					.substring(idx + 1)));
...
//			Calendar calendar = Calendar.getInstance(this.getTimeZone(), getAsLocale(this.getLocale()));
			JalaliCalendar calendar = new JalaliCalendar(this.getTimeZone(), getAsLocale(this.getLocale()));
			JalaliCalendar calendar2 = (JalaliCalendar) calendar.clone();
			calendar.setTime(dateRangeBegin);
			calendar2.setTime(dateRangeEnd);

			do {
				dates.add(calendar.getTime());
				calendar.add(JalaliCalendar.DATE, 1);
	...
	protected int getDefaultFirstWeekDay() {
		JalaliCalendar cal = getCalendar();
		return cal.getFirstDayOfWeek() - cal.getActualMinimum(JalaliCalendar.DAY_OF_WEEK);
	}
