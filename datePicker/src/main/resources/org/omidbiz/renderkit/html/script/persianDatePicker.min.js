...
(function () {
    jQuery.fn.persianDatepicker = function (options) {
        var pluginName = 'persianDatepicker';
        var instance = this.data(pluginName);
        if (!instance) {
            return this.each(function () {
                return jQuery(this).data(pluginName, new persianDatepicker(this, options));
            });
        }
    ...
    var persianDatepicker = (function () {
        function persianDatepicker(element, userOptions) {
            ...
            var el = self.el;
            self.options = jQuery.extend(false, {}, defaults, userOptions);
            ...
            self.headerStyle = "style='height:" + (_ch+10) + "px;line-height:" + (_ch+5) + "px; font-size:" + (_fontSize + 4) + "px;' ";
            ...
            self.calendar = jQuery('<div id="' + self._id + '" class="pdp-' + options.theme + '" />')
            ...
            jQuery(document).bind('mouseup', function (e) {
                ...
                var container = jQuery(".pdp-" + options.theme + " .yearSelect");
                if (!container.is(e.target) && container.has(e.target).length === 0) {
                    container.hide();
                }
                container = jQuery(".pdp-" + options.theme + " .monthSelect");
                if (!container.is(e.target) && container.has(e.target).length === 0) {
                    container.hide();
                }
            });
            ...
            jQuery('body').append(self.calendar);
            self.render();
            onResize();
        }
        ;
        ...
        persianDatepicker.prototype = {
            show: function () {
                this.calendar.show();
                //jQuery.each(jQuery('.pdp-el').not(this.el), function(i, o) {
                //    if (o.length) {
            ...
            header: function () {
                var self = this;
                _monthYear = jQuery('<div class="" />');
                _monthYear.appendTo(this.calendar);
                _head = jQuery('<div class="header" ' + self.headerStyle + ' />');
                _head.appendTo(this.calendar);
                _next = jQuery('<div class="nextArrow" />')
                ...
                var _monthSelect = jQuery('<ul class="monthSelect" ' + self.selectUlStyle + ' />').hide();
                var _yearSelect = jQuery('<ul class="yearSelect" ' + self.selectUlStyle + ' />').hide();
                // Build month label
                var _monthText = jQuery('<span/>')
                ...
                var getSelectableYears = function (f, l) {
                    ...
                    for (i = b; i < a && b > 0; i++)
                        arr.push(parseInt(i));
                    jQuery.each(self.options.selectableYears || ((pre) ? arr.reverse() : arr), function (i, v) {
                        var o = jQuery('<li ' + self.selectYearLiStyle + ' />').html(self.options.persianNumbers ? self.jDateFunctions.toPersianNums(v) : v);
                ...
                for (i = 1; i <= 12; i++) {
                    ...
                    var o = (self.options.selectableMonths._indexOf(i) == -1) ? jQuery('<li class="disableMonth" ' + self.selectMonthLiStyle + ' />').html(m) : jQuery('<li ' + self.selectMonthLiStyle + ' />').html(m);
                    ...
                    if (!o.hasClass('disableMonth')) {
                        o.bind("click", function () {
                            self.persianDate.date = 1;
                            self.persianDate.month = jQuery(this).data('month').monthNum;
                            self.render();
                        });
                    }
                ...
                _yearSelect.bind("scroll", function () {
                    if (self.options.selectableYears == undefined) {
                        c = jQuery(this).find("li").length;
                        firstYear = parseInt(jQuery(this).children("li:first").val());
                        lastYear = parseInt(jQuery(this).children("li:last").val());
                        lisHeight = c / 3 * (jQuery(this).find("li:first").height() + 4);
                        _com = jQuery(this).scrollTop().toString().length * 500;
                        if (jQuery(this).scrollTop() < _com.toString().length * 100 && firstYear >= 1) {
                            getSelectableYears(firstYear, 0);
                        }

                        _com = jQuery(this).scrollTop().toString().length * 100;
                        if ((lisHeight - jQuery(this).scrollTop()) > -_com && (lisHeight - jQuery(this).scrollTop()) < _com) {
                            getSelectableYears(0, lastYear);
                            jQuery(this).scrollTop(jQuery(this).scrollTop() - 50);
                        }
                        if (jQuery(this).scrollTop() < _com.toString().length && firstYear >= 30) {
                            jQuery(this).scrollTop(_com.toString().length * 100);
                        }
                    }
                });
                _monthYear.append(_monthSelect).append(_yearSelect);
                var titleYearMonth = jQuery('<div class="monthYear" />')
                        .append(_monthText)
                        .append("<span>&nbsp;&nbsp;</span>")
                        .append(_yearText);
                _head.append(titleYearMonth);
                _prev = jQuery('<div class="prevArrow" />')
            ...
            // days of week title
            dows: function () {
                _row = jQuery('<div class="dows" />');
                for (i = 0; i < 7; i++) {
                    _cell = jQuery('<div class="dow cell " ' + this.cellStyle + ' />')
                ...
                for (var row = 0, cellIndex = 0; row < 5 + 1; row++) {
                    ...
                    for (var col = 0; col < 7; col++, cellIndex++) {
                        if (cellIndex < _start || cellIndex - _start + 1 > _end) {
                            _cell = jQuery('<div class="nul cell " ' + self.cellStyle + ' />')
                                    .html('&nbsp;');
                        } else {
                            ...
                            if (self.options.selectedDate != undefined) {
                                ...
                                _selday = 'selday';
                            _fri = col == 6 ? 'friday' : '';
                            _cell = jQuery('<div class="day cell ' + _fri + ' ' + _today + ' ' + _selday + '" ' + self.cellStyle + ' />');
                            ...
                            _cell
                                    ...
                                    .bind("click", function () {
                                        ...
                                        jQuery(this).addClass("selday");
                                        if (self.options.showGregorianDate)
                                            self.showDate(self.el, jQuery(this).data('date').gDate);
                                        else
                                            self.showDate(self.el, jQuery(this).data('date').jDate);
                                        self.calendar.hide();
                                    });
                        }
                        _cell.appendTo(_row);
